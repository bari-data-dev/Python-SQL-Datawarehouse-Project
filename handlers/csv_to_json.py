...
def get_client_id(conn, client_schema):
    with conn.cursor() as cur:
        ...
        row = cur.fetchone()
        if not row:
            raise ValueError(f"Client schema '{client_schema}' tidak ditemukan.")
        return row[0]
    ...
    with conn.cursor() as cur:
        cur.execute("""
            INSERT INTO tools.file_audit_log (
                client_id, logical_source_file, physical_file_name, parquet_file_name,
                file_received_time, parquet_converted_time, total_rows,
        ...
        """, (
            ...
            kwargs["parquet_file_name"],
            kwargs["file_received_time"],
            kwargs["parquet_converted_time"],
            kwargs["total_rows"],
...
def insert_job_execution_log(conn, client_id, file_name, status, error_message=None, batch_id=None):
    with conn.cursor() as cur:
        ...
        """, (
            "csv_to_parquet.py", client_id, file_name, status,
...
# Main converter
# =========================
def convert_csv_to_parquet(client_schema, file_name, chunksize=100_000):
    ...
    parquet_converted_time = datetime.now()
    ...
    if not matched_logical_file:
        ...
        insert_job_execution_log(
            conn, client_id, file_name, "FAILED",
    ...
    try:
        ...
        for chunk in pd.read_csv(file_path, dtype=str, chunksize=chunksize):
            chunk = chunk.fillna("")
            chunk.insert(0, "csv_row_number", range(total_rows + 1, total_rows + len(chunk) + 1))
            chunk["source_file"] = file_name
            chunk["logical_source_file"] = matched_logical_file
            ...
            if first_chunk:
                ...
                chunk.to_parquet(output_path, engine="pyarrow", index=False, append=True)

            total_rows += len(chunk)

        print(f"✅ Parquet disimpan: {output_path} (rows={total_rows})")
        ...
        insert_file_audit_log(
            ...
            parquet_converted_time=parquet_converted_time,
...
if __name__ == "__main__":
    ...
    convert_csv_to_parquet(args.client, args.csv_file)
