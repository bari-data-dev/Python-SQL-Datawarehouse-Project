...
BEGIN
    ...
    EXECUTE format(
        'SELECT COUNT(*) FROM bronze_client1.erp_cust_az12 WHERE dwh_batch_id = %L',
        p_batch_id
    )
    INTO v_count;
    ...
    EXECUTE format(
        'DELETE FROM silver_client1.erp_cust_az12 WHERE dwh_batch_id = %L',
        p_batch_id
    );

    -- 4. Insert hasil transformasi
    EXECUTE format($sql$
        INSERT INTO silver_client1.erp_cust_az12 (
            cid,
            bdate,
            gen,
            dwh_batch_id
...
ALTER PROCEDURE tools.load_erp_cust_az12_v1(character varying, character varying)
    OWNER TO postgres;
